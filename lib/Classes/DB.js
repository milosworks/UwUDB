...
class uwuDB {
    ...
    */
    ...
    establecer(obj) {
        ...
        if (typeof obj !== 'object') throw new uwuDBError(`Debes de poner un objeto valido, se recibió '${typeof obj}'`)
     ...
     * @param {object} Búsqueda - Busca un doc con las propiedades puestas aquí
     * @returns {Documento}
     */
    buscarUno(Búsqueda) {
        if (!Búsqueda) throw new uwuDBError('Debes de poner una búsqueda para buscar')
        if (typeof Búsqueda !== 'object') throw new uwuDBError(`Debes de poner un objeto valido, se recibió '${typeof Búsqueda}'`)

        const BúsquedaProps = Utils.objProps(Búsqueda)
        const loosing = BúsquedaProps.properties.filter(item => !(this.Esquema.properties.includes(item)))

        if (loosing.some((item) => !this.Esquema.properties.includes(item))) throw new uwuDBError(`La(s) propiedad(es) '${loosing.join(', ')}' no existen en el esquema`)

        const sentence = this.db.prepare(`SELECT * FROM ${this.nombre} WHERE ${BúsquedaProps.values.join(', ')} LIMIT 1`).get()
     ...
     * @param {object|undefined} Búsqueda - Los datos que tiene que tener el documento para obtenerlo (como un buscarUno), si no se pone se darán todos los docs de toda la tabla
     * @param {number} limite - El limite de documentos que quieres obtener, si no se pone nada te dará todos los docs con esa Búsqueda
     * @returns {Documento[]}
     */
    buscar(Búsqueda, limite) {
        if (limite && typeof limite !== 'number') throw new uwuDBError(`Necesitas poner un limite valido, se recibió '${typeof limite}'`)
        if (!Búsqueda) {
        ...
        const BúsquedaProps = Utils.objProps(Búsqueda)
        if (!BúsquedaProps.properties.length) throw new uwuDBError('No pusiste ninguna propiedad a buscar')

        const loosing = BúsquedaProps.properties.filter(item => !(this.Esquema.properties.includes(item)))

        if (loosing.some((item) => !this.Esquema.properties.includes(item))) throw new uwuDBError(`La(s) propiedad(es) '${loosing.join(', ')}' no existen en el esquema`)

        const alldata = this.db.prepare(`SELECT * FROM ${this.nombre} WHERE ${BúsquedaProps.values.join(', ')}${limite ? ` LIMIT ${limite}` : ''}`).all()
     ...
     * @param {object} Búsqueda - El filtro para encontrar el documento a actualizar
     * @param {object} obj - Los nuevos datos de el documento
     * @returns {Documento} - El documento actualizado
     */
    actualizarUno(Búsqueda, obj) {
        const Doc = this.buscarUno(Búsqueda)
        ...
        this.db.prepare(`UPDATE ${this.nombre} SET ${UpdateProps.values.join(', ')} WHERE ${BúsquedaProps.values.join(', ')} LIMIT 1`).run()
     ...
     * @param {object} Filtro - El filtro para eliminar varios documentos, dejalo vació para eliminar todos los documentos de esta DB
     * @returns {{ok: boolean, documentos: number}}
     */
    eliminarVarios(Filtro) {
        ...
        if (!Docs.length) throw new uwuDBError('No hay ningún documento con ese filtro')
     ...
     * @returns {Promise<{ok: boolean, documentos: number}>} - Los documentos cambiados y si salio bien la operación
     */
    eliminarPorId(_id) {
        return new Promise((res, rej) => {
            ...
            if (!datos) rej(new uwuDBError(`No se encontró ningún documento con la id: ${_id}`))
    ...
    _checkAll(types, obj, rawObj) {
        ...
        for (const prop in rawObj) {
            ...
            if (typeof value !== type.type) throw new uwuDBError(`El tipo de la propiedad '${prop}' es incorrecto, se recibió '${typeof value}' y debería ser '${type.type}'`)
        }
    }
