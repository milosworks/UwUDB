...
class uwuDB {
    ...
    */
    ...
    buscarUno(query) {
        ...
        const QueryProps = Utils.objProps(query)
     ...
     * @param {object} query - El filtro para encontrar el documento a actulizar
     * @param {object} obj - Los nuevos datos de el documento
     */
    actualizarUno(query, obj) {
        const Doc = this.buscarUno(query)
        ...
        this.db.prepare(`UPDATE ${this.nombre} SET ${UpdateProps.values.join(', ')} WHERE ${QueryProps.values.join(', ')}`).run()
